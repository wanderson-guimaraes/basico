...
class adgalleryCMB_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		add_action( 'adgallerycmb_init_fields', array( &$this, 'init_fields' ) );
		...
		add_action( 'adgallerycmb_save_fields', array( &$this, 'save' ) );

		add_action( 'admin_enqueue_scripts', array( &$this, 'enqueue_scripts' ) );
		add_action( 'admin_enqueue_scripts', array( &$this, 'enqueue_styles' ) );

		add_filter( 'adgallerycmb_show_on', array( &$this, 'add_for_id' ), 10, 2 );
		add_filter( 'adgallerycmb_show_on', array( &$this, 'add_for_page_template' ), 10, 2 );
	...
	public function init_fields( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $key => $field ) {
			...
			$class = _adgallerycmb_field_class_for_type( $field['type'] );
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'adgallerycmb-scripts', trailingslashit( adgalleryCMB_URL ) . 'js/cmb.js', array( 'jquery' ) );
	...
	function enqueue_styles() {
		...
		$suffix = adgalleryCMB_DEV ? '' : '.min';

		if ( version_compare( get_bloginfo( 'version' ), '3.8', '>=' ) )
			wp_enqueue_style( 'adgallerycmb-styles', trailingslashit( adgalleryCMB_URL ) . "css/dist/cmb$suffix.css" );
		else
			wp_enqueue_style( 'adgallerycmb-styles', trailingslashit( adgalleryCMB_URL ) . 'css/legacy.css' );
	...
	function add() {
		...
		foreach ( (array) $this->_meta_box['pages'] as $page )
			if ( apply_filters( 'adgallerycmb_show_on', true, $this->_meta_box ) )
	...
	/**
	 * Show On Filters
	 * Use the 'adgallerycmb_show_on' filter to further refine the conditions under which a metabox is displayed.
	 ...
	 *
	 * @param  adgalleryCMB_Field[]  $fields
	 */
	static function layout_fields( array $fields ) { ?>

		<div class="adgallerycmb_metabox">
			...
			foreach ( $fields as $field ) :
				...
				<div class="adgallerycmb-cell-<?php echo intval( $field->args['cols'] ); ?>">
					
						<div <?php echo implode( ' ', $attrs ); ?>>
							<?php $field->display(); ?>
						</div>

						<input type="hidden" name="_adgallerycmb_present_<?php esc_attr_e( $field->id ); ?>" value="1" />
	...
	function strip_repeatable( $values ) {
		...
		foreach ( $values as $key => $value ) {
			...
			if ( false !== strpos( $key, 'adgallerycmb-group-x' ) || false !==  strpos( $key, 'adgallerycmb-field-x' ) )
				unset( $values[$key] ); 
	...
	function save( $post_id = 0 )  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! isset( $_POST['_adgallerycmb_present_' . $field['id'] ] ) )
				continue;
			...
			if ( ! $class = _adgallerycmb_field_class_for_type( $field['type'] ) ) {
				do_action('adgallerycmb_save_' . $field['type'], $field, $value);
			}
