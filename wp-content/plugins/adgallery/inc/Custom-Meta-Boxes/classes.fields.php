 ...
 * @abstract
 */
abstract class adgalleryCMB_Field {
	...
	public function __construct( $name, $title, array $values, $args = array() ) {
		...
		if ( ! empty( $this->args['std'] ) && empty( $this->args['default'] ) ) {
			...
			_deprecated_argument( 'adgalleryCMB_Field', "'std' is deprecated, use 'default instead'", '0.9' );
		}
	...
	public function get_the_id_attr( $append = null ) {
		...
		if ( isset( $this->parent ) ) {
			$parent_id = preg_replace( '/adgallerycmb\-field\-(\d|x)+/', 'adgallerycmb-group-$1', $this->parent->get_the_id_attr() );
			$id = $parent_id . '[' . $id . ']';
		}

		$id .= '-adgallerycmb-field-' . $this->field_index;
	...
	public function get_the_name_attr( $append = null ) {
		...
		if ( isset( $this->parent ) ) {
			$parent_name = preg_replace( '/adgallerycmb\-field\-(\d+|x)/', 'adgallerycmb-group-$1', $this->parent->get_the_name_attr() );
			$name = $parent_name . '[' . $name . ']';
		}

		$name .= "[adgallerycmb-field-$this->field_index]";
	...
	public function description() {
		...
		if ( $this->description ) { ?>
			...
			<div class="adgallerycmb_metabox_description"><?php echo wp_kses_post( $this->description ); ?></div>
	...
	public function display() {
		...
		foreach ( $values as $key => $value ) {
			...
			<?php if ( $this->args['repeatable'] ) : ?>
				<button class="adgallerycmb-delete-field" title="Remove field"><span class="adgallerycmb-delete-field-icon">&times;</span></button>
			<?php endif; ?>
		...
		if ( $this->args['repeatable'] ) {
			...
			<?php if ( $this->args['repeatable'] ) : ?>
				<button class="adgallerycmb-delete-field" title="Remove field"><span class="adgallerycmb-delete-field-icon">&times;</span> Remove Group</button>
			<?php endif; ?>
			...
			<button class="button adgallery-repeat-field"><?php esc_html_e( 'Add New', 'adgallerycmb' ); ?></button>
 ...
 * Standard text field.
 *
 * @extends adgalleryCMB_Field
 */
class adgalleryCMB_Text_Field extends adgalleryCMB_Field {
...
class adgalleryCMB_Text_Small_Field extends adgalleryCMB_Text_Field {

	public function html() { 

		$this->args['class'] .= ' adgallerycmb_text_small';	
 ...
 */
class adgalleryCMB_File_Field extends adgalleryCMB_Field {

	function enqueue_scripts() {

		parent::enqueue_scripts();
		wp_enqueue_media();
		wp_enqueue_script( 'adgallerycmb-file-upload', trailingslashit( adgalleryCMB_URL ) . 'js/file-upload.js', array( 'jquery', 'adgallerycmb-scripts' ) );
	...
	public function html() { 
		...
		<div class="adgallerycmb-file-wrap" <?php echo 'data-type="' . esc_attr( $data_type ) . '"'; ?>>

			<div class="adgallerycmb-file-wrap-placeholder"></div>

			<button class="button adgallerycmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>">
				<?php esc_html_e( 'Add File', 'adgallerycmb' ); ?>
			</button>

			<div class="adgallerycmb-file-holder type-file <?php echo $this->get_value() ? '' : 'hidden'; ?>">

				<?php if ( $this->get_value() ) : ?>

					<?php if ( isset( $icon_img ) ) echo $icon_img; ?>

					<div class="adgallerycmb-file-name">
			...
			<button class="adgallerycmb-remove-file button <?php echo $this->get_value() ? '' : 'hidden'; ?>">
				<?php esc_html_e( 'Remove', 'adgallerycmb' ); ?>
			</button>

			<input type="hidden" class="adgallerycmb-file-upload-input" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->value ); ?>" />
...
class adgalleryCMB_Image_Field extends adgalleryCMB_File_Field {
	...
	public function html() {
		...
		<div class="adgallerycmb-file-wrap" style="<?php echo esc_attr( $styles ); ?>" data-type="<?php echo esc_attr( $data_type ); ?>">

			<div class="adgallerycmb-file-wrap-placeholder" style="<?php echo esc_attr( $placeholder_styles ); ?>">
			...
			<button class="button adgallerycmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>" data-nonce="<?php echo wp_create_nonce( 'adgallerycmb-file-upload-nonce' ); ?>">
				<?php esc_html_e( 'Add Image', 'adgallerycmb' ); ?>
			</button>

			<div class="adgallerycmb-file-holder type-img <?php echo $this->get_value() ? '' : 'hidden'; ?>" data-crop="<?php echo (bool) $size['crop']; ?>">
	...
	static function request_image_ajax_callback() {
		...
		if ( ! ( isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'adgallerycmb-file-upload-nonce' ) ) )
			return;
...
add_action( 'wp_ajax_adgallerycmb_request_image', array( 'adgalleryCMB_Image_Field', 'request_image_ajax_callback' ) );

/**
 * Standard text meta box for a URL.
 *
 */
class adgalleryCMB_URL_Field extends adgalleryCMB_Field {

	public function html() { ?>

		<input type="text" <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'adgallerycmb_text_url code' ); ?> <?php $this->name_attr(); ?> value="<?php echo esc_attr( esc_url( $this->value ) ); ?>" />
 ...
 *
 */
class adgalleryCMB_Date_Field extends adgalleryCMB_Field {

	public function enqueue_scripts() {

		parent::enqueue_scripts();

		wp_enqueue_style( 'adgallerycmb-jquery-ui', trailingslashit( adgalleryCMB_URL ) . 'css/vendor/jquery-ui/jquery-ui.css', '1.10.3' );

		wp_enqueue_script( 'adgallerycmb-datetime', trailingslashit( adgalleryCMB_URL ) . 'js/field.datetime.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'adgallerycmb-scripts' ) );
	}

	public function html() { ?>

		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'adgallerycmb_text_small adgallerycmb_datepicker' ); ?> type="text" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->value ); ?>" />

	<?php }
}

class adgalleryCMB_Time_Field extends adgalleryCMB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'adgallerycmb-timepicker', trailingslashit( adgalleryCMB_URL ) . 'js/jquery.timePicker.min.js', array( 'jquery', 'adgallerycmb-scripts' ) );
		wp_enqueue_script( 'adgallerycmb-datetime', trailingslashit( adgalleryCMB_URL ) . 'js/field.datetime.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'adgallerycmb-scripts' ) );
	}

	public function html() { ?>

		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'adgallerycmb_text_small adgallerycmb_timepicker' ); ?> type="text" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->value ); ?>"/>
 ...
 *
 */
class adgalleryCMB_Date_Timestamp_Field extends adgalleryCMB_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'adgallerycmb_text_small adgallerycmb_datepicker' ); ?> type="text" <?php $this->name_attr(); ?>  value="<?php echo $this->value ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
 ...
 *
 */
class adgalleryCMB_Datetime_Timestamp_Field extends adgalleryCMB_Field {
	...
	public function html() { ?>		
		...
		<input <?php $this->id_attr('date'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'adgallerycmb_text_small adgallerycmb_datepicker' ); ?> type="text" <?php $this->name_attr( '[date]' ); ?>  value="<?php echo $this->value ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
		<input <?php $this->id_attr('time'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'adgallerycmb_text_small adgallerycmb_timepicker' ); ?> type="text" <?php $this->name_attr( '[time]' ); ?> value="<?php echo $this->value ? esc_attr( date( 'h:i A', $this->value ) ) : '' ?>" />
 ...
 */
class adgalleryCMB_Textarea_Field extends adgalleryCMB_Field {
 ...
 *  - int "rows" - number of rows in the <textarea>
 */
class adgalleryCMB_Textarea_Field_Code extends adgalleryCMB_Textarea_Field {
 ...
 *
 */
class adgalleryCMB_Color_Picker extends adgalleryCMB_Field {

	public function enqueue_scripts() {

		parent::enqueue_scripts();

		wp_enqueue_script( 'adgallerycmb-colorpicker', trailingslashit( adgalleryCMB_URL ) . 'js/field.colorpicker.js', array( 'jquery', 'wp-color-picker', 'adgallerycmb-scripts' ) );
		wp_enqueue_style( 'wp-color-picker' );
	}

	public function html() { ?>

		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'adgallerycmb_colorpicker adgallerycmb_text_small' ); ?> type="text" <?php $this->name_attr(); ?> value="<?php echo esc_attr( $this->get_value() ); ?>" />
 ...
 */
class adgalleryCMB_Radio_Field extends adgalleryCMB_Field {
 ...
 *
 */
class adgalleryCMB_Checkbox extends adgalleryCMB_Field {
 ...
 *
 */
class adgalleryCMB_Title extends adgalleryCMB_Field {
 ...
 *
 */
class adgalleryCMB_wysiwyg extends adgalleryCMB_Field {

	function enqueue_scripts() {

		parent::enqueue_scripts();

		wp_enqueue_script( 'adgallerycmb-wysiwyg', trailingslashit( adgalleryCMB_URL ) . 'js/field-wysiwyg.js', array( 'jquery', 'adgallerycmb-scripts' ) );
	}
	...
	public function html() { 
		...
		printf( '<div class="adgallerycmb-wysiwyg" data-id="%s" data-name="%s" data-field-id="%s">', $id, $name, $field_id );
	
		if ( $this->is_placeholder() ) 	{

			// For placeholder, output the markup for the editor in a JS var.
			ob_start();
			$this->args['options']['textarea_name'] = 'adgallerycmb-placeholder-name-' . $field_id;
			wp_editor( '', 'adgallerycmb-placeholder-id-' . $field_id, $this->args['options'] );
			$editor = ob_get_clean();
			...
			<script>
				if ( 'undefined' === typeof( adgallerycmb_wysiwyg_editors ) ) 
					var adgallerycmb_wysiwyg_editors = {};
				adgallerycmb_wysiwyg_editors.<?php echo $field_id; ?> = '<?php echo $editor; ?>';
			</script>
 ...
 */
class adgalleryCMB_Select extends adgalleryCMB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'select2', trailingslashit( adgalleryCMB_URL ) . 'js/select2/select2.js', array( 'jquery' ) );
		wp_enqueue_script( 'field-select', trailingslashit( adgalleryCMB_URL ) . 'js/field.select.js', array( 'jquery', 'select2', 'adgallerycmb-scripts' ) );
	}
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'select2', trailingslashit( adgalleryCMB_URL ) . 'js/select2/select2.css' );
	}
	...
	public function output_field() {
		...
		<select 
			...
			class="adgallerycmb_select" 
			style="width: 100%" 
		>

			<?php if ( ! empty( $this->args['allow_none'] ) ) : ?>
				<option value=""><?php echo esc_html_x( 'None', 'select field', 'adgallerycmb' ) ?></option>
			<?php endif; ?>
	...
	public function output_script() {
		...
		<script type="text/javascript">
			...
			(function($) {
				...
				options.placeholder = <?php echo json_encode( __( 'Type to search', 'adgallerycmb' ) ) ?>;
				options.allowClear  = true;

				if ( 'undefined' === typeof( window.adgallerycmb_select_fields ) )
					window.adgallerycmb_select_fields = {};
				
				var id = <?php echo json_encode( $this->get_js_id() ); ?>;
				window.adgallerycmb_select_fields[id] = options;
...
class adgalleryCMB_Taxonomy extends adgalleryCMB_Select {
 ...
 *     'multiple'     => bool whether multiple can be selected
 */
class adgalleryCMB_Post_Select extends adgalleryCMB_Select {
	...
	public function output_script() {
		...
		<script type="text/javascript">
			...
			(function($) {
				...
				var id = <?php echo json_encode( $this->get_js_id() ); ?>;
				var options = window.adgallerycmb_select_fields[id];
				...
				<?php if ( $this->args['ajax_url'] ) : ?>
					...
					var ajaxData = {
						action  : 'adgallerycmb_post_select',
						post_id : '<?php echo intval( get_the_id() ); ?>', // Used for user capabilty check.
						nonce   : <?php echo json_encode( wp_create_nonce( 'adgallerycmb_select_field' ) ); ?>,
...
function adgallerycmb_ajax_post_select() {
	...
	if ( ! $nonce || ! wp_verify_nonce( $nonce, 'adgallerycmb_select_field' ) || ! current_user_can( 'edit_post', $post_id ) ) {
...
add_action( 'wp_ajax_adgallerycmb_post_select', 'adgallerycmb_ajax_post_select' );
 ...
 */
class adgalleryCMB_Group_Field extends adgalleryCMB_Field {
	...
	function __construct() {
		...
		if ( ! empty( $this->args['fields'] ) ) {
			foreach ( $this->args['fields'] as $f ) {
				...
				$class = _adgallerycmb_field_class_for_type( $f['type'] );
				$f['show_label'] = true;
	...
	public function display() {
		...
		if ( $this->args['repeatable'] ) {
			...
			$this->value = ''; ?>
				...
				<button class="button adgallery-repeat-field"><?php esc_html_e( 'Add New Group', 'adgallerycmb' ); ?></button>
	...
	public function html() {
		...
		<?php adgalleryCMB_Meta_Box::layout_fields( $fields ); ?>
	...
	public function add_field( adgalleryCMB_Field $field ) {
		$field->parent = $this;
