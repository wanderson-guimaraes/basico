...
class GroupFieldTestCase extends WP_UnitTestCase {
	...
	function testAddField() {
		...
		$group  = new adgalleryCMB_Group_Field( 'group', 'Group Title', array() );
		$field1 = new adgalleryCMB_Text_Field( 'foo', 'Title', array( 1 ) );
		$field2 = new adgalleryCMB_Text_Field( 'bar', 'Title', array( 2, 3 ), array( 'repeatable' => true ) );
	...
	function testGetValues() {
		...
		$group  = new adgalleryCMB_Group_Field( 'group', 'Group Title', array() );
		$field1 = new adgalleryCMB_Text_Field( 'foo', 'Title', array() );
		$field2 = new adgalleryCMB_Text_Field( 'bar', 'Title', array() );
	...
	function testFieldNameAttrValue() {
		...
		$group  = new adgalleryCMB_Group_Field( 'group', 'Group Title', array() );
		$field1 = new adgalleryCMB_Text_Field( 'foo', 'Title', array( 1, 2 ) );

		$group->add_field( $field1 );

		// Standard use of ID attribute
		$id_attr = $field1->get_the_name_attr();
		$this->assertEquals( $id_attr, 'group[adgallerycmb-group-0][foo][adgallerycmb-field-0]' );

		// Using append
		$id_attr = $field1->get_the_name_attr( '[bar]' );
		$this->assertEquals( $id_attr, 'group[adgallerycmb-group-0][foo][adgallerycmb-field-0][bar]' );

		// Test repeatable group.
		$group->field_index = 1;
		$id_attr = $field1->get_the_name_attr();
		$this->assertEquals( $id_attr, 'group[adgallerycmb-group-1][foo][adgallerycmb-field-0]' );
		$group->field_index = 0; // Unset

		// Test repeatable field within group.
		$field1->field_index = 1;
		$id_attr = $field1->get_the_name_attr();
		$this->assertEquals( $id_attr, 'group[adgallerycmb-group-0][foo][adgallerycmb-field-1]' );
		...
		$id_attr = $field1->get_the_name_attr();
		$this->assertEquals( $id_attr, 'group[adgallerycmb-group-12][foo][adgallerycmb-field-0]' );
		$group->field_index = 0; // Unset
